...
capture log close
clear
log using "$pathlog/05_shocks_2011", replace
set more off

* Load in shock module to look at different shocks faced by hh
use "$wave3/GSEC16.dta", clear
...
* Create bar graph of shocks ranked by severity
	...
	*/, size(small) color("100 100 100"))) 
	graph export "$pathgraph\HazCope2011.pdf", as(pdf) replace	
...
* Save shock data
save "$pathout/shocks_2011.dta", replace
