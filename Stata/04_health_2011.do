...
clear
capture log close
log using "$pathlog/04_health_2011", replace
di in yellow "`c(current_date)' `c(current_time)'"

u "$wave3/GSEC5.dta", replace
...
ds(h2* h5*), not
keep `r(varlist)'
merge 1:1 HHID PID using "$pathout/hhchar_ind_2011.dta
...
use "$wave3/GSEC6A.dta", clear

* Merge in hh gender information
merge 1:1 HHID PID using "$wave3/GSEC2.dta"
keep if _merge == 3
drop _merge
...
*/ xlabel(0(6)60,  labsize(small)) title("Child Nutrition Outcomes: 2011 (unweighted)")
...
merge 1:1 HHID using "$pathout/healthtmp_2011.dta"
